...
public class IzumikSkillbarOverlay {
	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
        ...
        if (!world.getEntitiesOfClass(IzumikEntity.class, AABB.ofSize(new Vec3(x, y, z), 64, 64, 64), e1 -> true).isEmpty()) {
            ...
            double ind = 0;
            double phase = 0;
            ent = (Entity) world.getEntitiesOfClass(IzumikEntity.class, AABB.ofSize(new Vec3(x, y, z), 64, 64, 64), e -> true).stream().sorted(new Object() {
            ...
            event.getGuiGraphics().blit(new ResourceLocation(CaerulaArborMod.MODID, "textures/screens/izumik_skillbar.png"), 13, h / 2 + -48, Mth.clamp((int) ind * 4, 0, 400), 0, 4, 100, 404, 100);
