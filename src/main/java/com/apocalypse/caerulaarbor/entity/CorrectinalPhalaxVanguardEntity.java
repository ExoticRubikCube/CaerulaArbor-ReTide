...
public class CorrectinalPhalaxVanguardEntity extends Animal implements GeoEntity {
                    ...
                    if (!(enemy == null)) {
                        if ((enemy != null ? distanceTo(enemy) : -1) <= 5 && enemy.isAlive()) {
                            ...
                            if (this instanceof CorrectinalPhalaxVanguardEntity) {
                                ((CorrectinalPhalaxVanguardEntity) this).setAnimation("animation.correctional_phalanx _vanguard.swing");
                            }
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 45) {
			this.remove(CorrectinalPhalaxVanguardEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
