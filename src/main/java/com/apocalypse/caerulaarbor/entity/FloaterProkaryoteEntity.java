...
public class FloaterProkaryoteEntity extends SeaMonster implements RangedAttackMob {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(9, new RandomStrollGoal(this, 1, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = FloaterProkaryoteEntity.this.getRandom();
				double dir_x = FloaterProkaryoteEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = FloaterProkaryoteEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = FloaterProkaryoteEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}
		});
		this.goalSelector.addGoal(10, new RandomLookAroundGoal(this));
		this.goalSelector.addGoal(1, new FloaterProkaryoteEntity.RangedAttackGoal(this, 1.25, 60, 5f) {
			@Override
			public boolean canContinueToUse() {
				return this.canUse();
			}
	...
	public static void init() {
		SpawnPlacements.register(CaerulaArborModEntities.FLOATER_PROKARYOTE.get(), SpawnPlacements.Type.IN_WATER, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
