...
public class IllusionerBannerBlockModel extends GeoModel<IllusionerBannerTileEntity> {
	...
	public ResourceLocation getAnimationResource(IllusionerBannerTileEntity animatable) {
		...
		if (blockstate == 1)
			return ResourceLocation.fromNamespaceAndPath(CaerulaArbor.MODID, "animations/oceanized_banner.animation.json");
		return ResourceLocation.fromNamespaceAndPath(CaerulaArbor.MODID, "animations/oceanized_banner.animation.json");
	}
	...
	public ResourceLocation getModelResource(IllusionerBannerTileEntity animatable) {
		final int blockstate = animatable.blockstateNew;
		if (blockstate == 1)
			return ResourceLocation.fromNamespaceAndPath(CaerulaArbor.MODID, "geo/oceanized_banner.geo.json");
		return ResourceLocation.fromNamespaceAndPath(CaerulaArbor.MODID, "geo/oceanized_banner.geo.json");
	}
	...
	public ResourceLocation getTextureResource(IllusionerBannerTileEntity animatable) {
		final int blockstate = animatable.blockstateNew;
		if (blockstate == 1)
			return ResourceLocation.fromNamespaceAndPath(CaerulaArbor.MODID, "textures/block/illusioner_banner.png");
		return ResourceLocation.fromNamespaceAndPath(CaerulaArbor.MODID, "textures/block/illusioner_banner.png");
	}
}